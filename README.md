# Paradex Testnet Trading Bot — Step 1/2 Validation

**Status: full lifecycle confirmed live on Paradex testnet** (2026-07-24). Placed a BUY
MARKET/IOC order on BTC-USD-PERP, confirmed the fill, saw the position open with live
...
Validates the full Paradex testnet trade lifecycle end-to-end before any UI work starts:

```
Wallet -> Testnet funding -> Paradex account (onboarding) -> Market data
-> Place order -> Order accepted -> Trade executed -> Position opened
-> Monitor position/P&L -> Close position -> Trade history
```

Uses the official [`paradex-py`](https://github.com/tradeparadex/paradex-py) SDK rather than
hand-rolled request signing, so the auth/signature scheme always matches whatever Paradex
currently implements (see "Why the SDK, not raw REST" below).
...
Paradex's `/onboarding` endpoint rejects brand-new wallets with
...
`paradex_py` can onboard it. Get testnet ETH from:
...
Once funded, the Paradex account itself is auto-credited **~$100,000 test USDC** on
...
because of two live Paradex-side issues discovered while building this — see
"Two Paradex-side bugs found" below before assuming this is a normal SDK step.
...
`postman/Paradex_Testnet.postman_collection.json` covers the same endpoints:
...
Paradex auth is a SIWE (EIP-191) signature challenge, which Postman can't produce on its
...
## Two Paradex-side bugs found, and how this project routes around them

**Bug 1 — the "native Starknet account" onboarding path is currently broken on testnet.**
`Paradex(env=TESTNET, l1_address=..., l1_private_key=...)` — the constructor shown in the
SDK's own top-level example — derives a Starknet account the legacy way and tries to
deploy it on-chain using the contract class in `system/config`'s `paraclear_account_proxy_hash`.
That class (`0x03530cc4759d78042f1b543bf797f5f3d647cde0388c33734cf91b7f7b9314a9`) is **not
declared** on Paradex's testnet chain right now — confirmed directly via
`starknet_getClass` against `rpc.api.testnet.paradex.trade`, independent of any SDK
version (checked against `paradex-py` 0.6.3, the latest on PyPI). Every deploy attempt via
this path reverts with `Class ... is not declared`. This blocks *any* new account trying to
onboard this way, not just this project's wallet.

**Workaround:** `ParadexEvm` (SIWE/EIP-191 auth) uses a *different* declared class
(`paraclear_evm_account_hash`) and worked cleanly — account created, `$100,001` test USDC
credited, deploy tx `PRE_CONFIRMED` (not reverted). This is why `config.py` uses
`ParadexEvm` for onboarding instead of the legacy `Paradex` constructor.

**Bug 2 — `ParadexEvm.create_trading_subkey()` is broken against the live API.** An
EVM-authenticated account has no Starknet signing key of its own, so it can't sign orders
— Paradex's fix is registering a "subkey" (a real Starknet keypair) for order signing. The
SDK's convenience helper for this omits fields the server now requires
(`INVALID_REQUEST_PARAMETER: evm_signature is required for EIP-191 accounts`) — confirmed
against both the released 0.6.3 package and the current GitHub `main` branch, so it's not
yet fixed upstream either.

**Workaround:** `scripts/01_register_subkey.py` builds the required SIWE message by hand
(`Statement` must read `Paradex Subkey Registration: 0x<pubkey>`, lowercased, plus an
`Expiration Time` line — both undocumented in the SDK, found by iterating on the API's
error messages) and calls the low-level `create_subkey()` method directly, bypassing the
...
Net effect: this project never had to wait on Paradex to fix Bug 1 — the whole lifecycle
runs on `ParadexEvm` (onboarding/reads) + a hand-registered `ParadexSubkey` (trading),
entirely within paradex-py's public API surface, just not the paths its own docs lead with.
...
flow is EIP-191/SIWE-based (see `paradex_py/account/evm_account.py`), and the exact
market config field names differ too (`order_size_increment`/`price_tick_size`, not
`step_size`/`tick_size`). Building on the official SDK avoids silently drifting from
whatever Paradex's API actually does today.

## Step 3 — the UI (built and verified)

```
React (Vite + TS, dark theme) -> FastAPI (backend/) -> paradex_py -> Paradex Testnet
```
...
- **`backend/`** — FastAPI wrapper around the same `paradex_py` client used above
  (imports `scripts/config.py` directly, one source of truth for auth). REST for

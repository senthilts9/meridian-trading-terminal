# Testnet Trading Terminal — Runbook & Architecture

**Author:** Senthil Saravanamuthu
**Status:** Full trade lifecycle verified live on the exchange's testnet (2026-07-24)
...
A complete, automated crypto perpetuals trade lifecycle against a live testnet, using a
self-custodial wallet with no manual browser trading:
...
Fund with testnet ETH (Sepolia faucet — anti-Sybil gate, not trading capital)
        |
        v
Exchange account (SIWE onboarding) ---> auto-credited ~$100,000 test USDC
        |
        v
...
Read live market data (orderbook, BBO, mark price)
        |
        v
Place order (signed by the subkey) ---> matched by the exchange's engine
        |
        v
...
Every arrow above is a real HTTP call to the exchange's testnet API, not a mock.
...
|                                                                    |
|  scripts/*.py  --uses-->  Exchange SDK  --HTTPS-->  Exchange API |
|       |                                                testnet    |
|       v                                                            |
...
| Client | Auth method | Can read account/positions/orders? | Can sign orders? | Can withdraw? |
|---|---|---|---|---|
| Main account | SIWE/EIP-191 (EVM) | Yes | **No** | Yes |
| Trading subkey | Starknet keypair | Yes | **Yes** | No |
...
order-signing logic. This is the exchange's actual security model, not a shortcut we
invented.

### 2.2 Planned state (Step 3 — UI, next)

```
React/Vite dark-themed UI  <-- REST/WS -->  FastAPI backend  <-- REST/WS -->  Exchange testnet
...
only to our backend, never directly to the exchange, and never sees a private key.
...
| `/orders/by_client_id/{client_id}` | GET | Same, looked up by our own idempotency key instead of the exchange's order id | `04_check_order.py` |
...
## 5. Two exchange-side bugs found (read before debugging auth failures yourself)
...
1. **Legacy native-Starknet onboarding is broken on testnet right now.** The account-deploy
   transaction reverts because the contract class the exchange's own `/system/config` tells
...
2. **The SDK's subkey-registration helper is broken against the live API** — it omits the
   `evm_signature`/`siwe_message` fields the server now requires for EIP-191 accounts.
   Confirmed against both the released package and GitHub `main`.

This project routes around both (see `scripts/config.py` and `scripts/01_register_subkey.py`)
by using the EVM/SIWE client + a hand-registered subkey client instead of the paths the
SDK's own top-level examples lead with.
...
```
React (Vite + TS, dark theme)  <--REST + WS-->  FastAPI (backend/)  <--REST-->  Exchange testnet
```

The backend (`backend/main.py`) is a thin wrapper: every endpoint just calls the same
exchange client used by the scripts (imported directly from `scripts/config.py`, so
...
every 2s and pushes the snapshot), not a direct bridge to the exchange's own WS channel
protocol. This was the pragmatic choice for a v1 — correct data, simple to reason about —
rather than implementing the exchange's native WS subscription/SBE handling. A
closer-to-the-metal version would subscribe to those WS channels directly and relay ticks
as they arrive instead of polling; worth doing if latency matters for a real strategy.
